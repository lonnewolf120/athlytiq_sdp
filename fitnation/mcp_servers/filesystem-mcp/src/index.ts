  ...
  try {
    switch (name) {
      ...
      case 'read_multiple_files': {
        ...
        const rejected = results.filter(r => r.status === 'rejected').map(r => ({ path: (r as PromiseRejectedResult).reason?.path || 'unknown', error: (r as PromiseRejectedResult).reason?.message || 'unknown error' }));
      ...
      case 'directory_tree': {
        ...
        async function buildTree(currentPath: string, indent: string) {
          const entries = await fs.readdir(currentPath, { withFileTypes: true });
          for (const entry of entries) {
            const fullPath = path.join(currentPath, entry.name);
            try {
