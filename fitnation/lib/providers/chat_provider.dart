...
class ChatProvider with ChangeNotifier {
  ...
  void _handleMessageReadReceipt(Map<String, dynamic> data) {
    ...
    for (int i = 0; i < _messages.length; i++) {
      if (_messages[i].roomId == roomId && messageIds.contains(_messages[i].id)) {
  ...
  // Send a message
  Future<void> sendMessage(String roomId, String content, {String? replyToId}) async {
    try {
      ...
      // Send via WebSocket
      _webSocketService.sendTextMessage(roomId, content, replyToId: replyToId);
  ...
  void _addMessage(ChatMessage message) {
    ...
    _messages.removeWhere((m) => m.id == 'temp_${message.createdAt.millisecondsSinceEpoch}');
