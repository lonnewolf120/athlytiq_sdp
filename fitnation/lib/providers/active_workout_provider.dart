...
import 'package:fitnation/models/Exercise.dart'
    as exercise_db; // From ExerciseDB
...
class ActiveWorkoutNotifier extends StateNotifier<ActiveWorkoutState> {
  ...
  void startWorkout({String? name}) {
    debugPrint(
      "ActiveWorkoutNotifier: startWorkout called. Current state startTime: ${state.startTime}",
    );
    ...
    state = state.copyWith(
      ...
      workoutName:
          name ??
          'Workout Session - ${DateTime.now().toIso8601String().substring(0, 10)}',
      clearEndTime: true, // Ensure endTime is cleared if restarting
      intensityScore: null, // Clear previous intensity
    );
    debugPrint(
      "ActiveWorkoutNotifier: startWorkout finished. New state startTime: ${state.startTime}",
    );
  }
  ...
  void removeExercise(String activeExerciseId) {
    state = state.copyWith(
      exercises:
          state.exercises.where((ex) => ex.id != activeExerciseId).toList(),
    );
  }
  ...
  void finishWorkout(double intensity) {
    debugPrint(
      "ActiveWorkoutNotifier: finishWorkout called. Current state startTime: ${state.startTime}",
    );
    if (state.startTime == null) {
      debugPrint(
        "ActiveWorkoutNotifier: ERROR in finishWorkout - Workout not started (startTime is null).",
      );
      return;
    }
    final newEndTime = DateTime.now();
    state = state.copyWith(endTime: newEndTime, intensityScore: intensity);
    debugPrint(
      "ActiveWorkoutNotifier: finishWorkout finished. New state endTime: ${state.endTime}, intensityScore: ${state.intensityScore}",
    );
  }
  ...
  CompletedWorkout? generateCompletedWorkoutData(String userId) {
    ...
    if (!state.isFinished ||
        ...
        state.duration == null ||
        state.intensityScore == null) {
      debugPrint(
        "generateCompletedWorkoutData: Condition 1 FAILED - Workout not ready (isFinished, startTime, duration, or intensityScore is null/false).",
      );
    ...
    for (var activeEx in state.exercises) {
      debugPrint(
        "generateCompletedWorkoutData: Processing ActiveExercise ID: ${activeEx.id}, Name: ${activeEx.baseExercise.name}",
      );
      activeEx.sets.asMap().forEach((idx, s) {
        debugPrint(
          "  Set $idx for ${activeEx.baseExercise.name}: id=${s.id}, weight='${s.weight}', reps='${s.reps}', isCompleted=${s.isCompleted}",
        );
      });
      // Prefer completed sets; if none are marked completed, fall back to including all existing sets
      List<CompletedWorkoutSet> completedSets =
          activeEx.sets
              .where((s) => s.isCompleted)
              .map(
                (s) => CompletedWorkoutSet(
                  id: s.id,
                  weight: s.weight,
                  reps: s.reps,
      ...
      if (completedSets.isEmpty && activeEx.sets.isNotEmpty) {
        debugPrint(
          "  No sets marked completed for ${activeEx.baseExercise.name}. Falling back to include all sets.",
        );
        completedSets =
            activeEx.sets
                .map(
      ...
      debugPrint(
        "  Using ${completedSets.length} sets for exercise ${activeEx.baseExercise.name} in completed workout data.",
      );

      // Add exercise if it has any sets after fallback
    ...
    // Allow saving even if there are no completed exercises; an empty list is acceptable
    debugPrint(
      "generateCompletedWorkoutData: All conditions passed. Generating CompletedWorkout object.",
    );
    return CompletedWorkout(
      ...
      createdAt:
          DateTime.now(), // Timestamp for the CompletedWorkout record itself
    );
  }

  void resetWorkout() {
    debugPrint(
      "ActiveWorkoutNotifier: resetWorkout called. Current state startTime: ${state.startTime}",
    );
    state = ActiveWorkoutState();
    debugPrint(
      "ActiveWorkoutNotifier: resetWorkout finished. New state startTime: ${state.startTime}",
    );
  }
  ...
  void loadState(ActiveWorkoutState stateToLoad) {
    debugPrint(
      "ActiveWorkoutNotifier: loadState called. Loading state with startTime: ${stateToLoad.startTime}",
    );
    state = stateToLoad;
  }
  ...
  void initializeNewWorkout({
    String? name,
    List<ActiveWorkoutExercise>? initialExercises,
  }) {
    debugPrint(
      "ActiveWorkoutNotifier: initializeNewWorkout called. Name: $name",
    );
    state = ActiveWorkoutState(
  ...
  void restartWorkout({String? name}) {
    debugPrint(
      "ActiveWorkoutNotifier: restartWorkout called. Current state startTime: ${state.startTime}",
    );
    ...
    final List<ActiveWorkoutExercise> restartedExercises =
        state.exercises.map((activeEx) {
          return ActiveWorkoutExercise(
    ...
    state = state.copyWith(
      ...
      workoutName:
          name ??
          state.workoutName, // Keep current name if new one isn't provided
      clearEndTime: true,
      intensityScore: null, // Clear previous intensity
      exercises: restartedExercises,
    );
    debugPrint(
      "ActiveWorkoutNotifier: restartWorkout finished. New state startTime: ${state.startTime}",
    );
  }

  Future<bool> saveCompletedWorkout(
    String userId,
    ...
    if (completedWorkoutData == null) {
      debugPrint(
        "ActiveWorkoutNotifier: No valid completed workout data to save.",
      );
      return false;
    }
    ...
    try {
      debugPrint(
        "ActiveWorkoutNotifier: Attempting to save workout session to API.",
      );
      // Attempt to save to API
      await apiService.saveWorkoutSession(completedWorkoutData);
      debugPrint(
        "ActiveWorkoutNotifier: Workout session saved to API successfully.",
      );
      ...
      debugPrint(
        "ActiveWorkoutNotifier: Workout session saved to local DB as synced.",
      );
      resetWorkout(); // Reset state after successful save
      return true;
    } catch (e) {
      debugPrint(
        "ActiveWorkoutNotifier: Failed to save workout session to API: $e. Saving locally as unsynced.",
      );
      try {
        // If API save fails, save to local DB as unsynced
        await dbHelper.insertCompletedWorkout(
          completedWorkoutData,
          synced: false,
        );
        debugPrint(
          "ActiveWorkoutNotifier: Workout session saved to local DB as unsynced.",
        );
        resetWorkout(); // Reset state after successful local save
        return true; // Still true as it's saved locally
      } catch (dbError) {
        debugPrint(
          "ActiveWorkoutNotifier: Failed to save workout session to local DB: $dbError",
        );
        return false;
      }
    }
